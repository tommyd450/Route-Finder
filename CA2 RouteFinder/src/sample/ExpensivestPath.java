...
public class ExpensivestPath
{
    ...
    public static <T> CostedPath searchGraphDepthFirstExpensivePath(GraphNodeAL<?> from, ArrayList<GraphNodeAL<?>> encountered, int totalCost, T lookingfor)
    {
        ...
        for (GraphLinkAL adjLink : from.adjList) //For every adjacent node
            if (!encountered.contains(adjLink.dest)) //That has not yet been encountered
            {
                ...
                CostedPath temp = searchGraphDepthFirstExpensivePath(adjLink.dest, new ArrayList<>(encountered),
